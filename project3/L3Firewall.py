...
class Firewall (EventMixin):
	...
	def __init__ (self,l2config,l3config):
		...
		# Anthony Pipia
		# ==== NEW CODE:
		self.port_table = {} #F.SrcMAC -> F.SrcIP
		# ====== END NEW CODE
	...
	def _handle_PacketIn(self, event):
	    ...
	    # Extract IP if this is an IP packet
	    src_ip = None
	    if match.dl_type == packet.IP_TYPE:
	        ...
	        if src_mac not in self.port_table:
	            ...
	            if self.port_table[src_mac] != src_ip:
	                # This MAC is using multiple IPs → spoofing!
	                log.warn("Spoofing detected! MAC %s used IP %s but originally had %s",
	                         src_mac, src_ip, self.port_table[src_mac])
	                
	                # INSTALL DROP RULE ON SWITCH
	                fm = of.ofp_flow_mod()
	                fm.match = of.ofp_match(dl_src = src_mac)
	                fm.actions = []     # empty actions = DROP
	                event.connection.send(fm)

                return  # stop processing this packet entirely
		# ====== END NEW CODE
